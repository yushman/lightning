# test-distribution — target contract (deferred)

> **Status: deferred.** This change is design-only and is never archived (phase-5 DoD), so these requirements never land in `openspec/specs/` through it. They are the target contract the future stage-1..3 implementation changes will carry (and may refine) in their own delta specs. Stage mapping per requirement is noted inline.
