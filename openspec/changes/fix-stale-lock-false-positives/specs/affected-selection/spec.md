...
The invalidation hash SHALL cover, root-relatively and skipping `.git`/`.gradle`/`build` directories: `**/*.gradle`, `**/*.gradle.kts`, `buildSrc/**`, `build-logic/**`, `gradle/libs.versions.toml`, `gradle/wrapper/gradle-wrapper.properties`, `gradle.properties`, `local.properties`, globs from `[affected] invalidate_on` in `lightning.toml`, plus `<dir>/**` for every directory recorded in the lock's `included_builds`. `lightning affected` and `lightning run` SHALL treat the lock as stale solely when the recomputed hash differs from the stored one; the diff content SHALL NOT by itself mark the lock stale. A missing or stale lock SHALL abort with exit code 4 and a message pointing at `lightning sync`, unless `--auto-sync` is given, in which case sync runs first and selection proceeds. The lock format version SHALL be 2; a lock with any other version SHALL be rejected with an error instructing to run `lightning sync`.
